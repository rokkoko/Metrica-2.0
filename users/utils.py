...
def get_player_calendar_with_week_day_number(game_session_objs_list):
    """
    :param game_session_objs_list: object of a model with game sessions of user with annotated played weekdays
    :return: dict(number of weekday : percentage of play for that day)
    """
    # "No-query" operation. Queryset already cached in variable
    list_of_played_weekdays = [elem.weekday for elem in game_session_objs_list]
...
def get_player_calendar_with_week_day_name(game_session_objs_list):
    ...
    #  result just with int-repr for each weekday
    return dict(counter)
